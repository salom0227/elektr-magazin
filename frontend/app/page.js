    ...
    <div className="min-h-screen" style={{ background: '#f0f4f8' }}>
      ...
      <div className="relative overflow-hidden py-20 px-4"
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-6 animate-fadeIn"
            style={{ background: 'rgba(59,130,246,0.2)', border: '1px solid rgba(59,130,246,0.4)', color: '#93c5fd' }}>
            ⚡ Sifatli elektr aksessuarlar
          </div>
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 animate-fadeUp leading-tight">
          ...
          <p className="text-blue-200 text-xl mb-10 animate-fadeUp" style={{ animationDelay: '0.2s' }}>
            Rozetka, adapter, kabel va boshqa sifatli mahsulotlar
          </p>
          <div className="flex gap-4 justify-center animate-fadeUp" style={{ animationDelay: '0.4s' }}>
            <Link href="/catalog"
              className="btn-glow px-8 py-4 rounded-2xl font-bold text-gray-900 text-lg transition-all"
          ...
          {/* Stats */}
          <div className="grid grid-cols-3 gap-6 mt-16 max-w-lg mx-auto animate-fadeUp" style={{ animationDelay: '0.6s' }}>
            {[
              { n: '500+', l: 'Mahsulot' },
              { n: '1000+', l: 'Mijozlar' },
              { n: '24/7', l: 'Xizmat' },
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 mb-14">
          {categories.map((c, i) => (
            <Link key={c.id} href={`/catalog?category=${c.id}`}
              className="category-card bg-white rounded-2xl p-4 text-center cursor-pointer"
              style={{ animationDelay: `${i * 0.1}s`, border: '1px solid #e2e8f0' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {products.map((p, i) => (
            <div key={p.id} className="card-3d bg-white rounded-2xl overflow-hidden"
              style={{ border: '1px solid #e2e8f0', animationDelay: `${i * 0.1}s` }}>
              ...
              <div className="p-4">
                ...
                <Link href={`/product/${p.slug}`}>
                  <p className="font-semibold text-gray-800 text-sm mt-2 hover:text-blue-600 transition-colors line-clamp-2 leading-snug">
                    {p.name}
                  </p>
                </Link>
                ...
                <button onClick={() => handleAdd(p)}
                  className="w-full mt-3 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 btn-glow"
                  style={{
